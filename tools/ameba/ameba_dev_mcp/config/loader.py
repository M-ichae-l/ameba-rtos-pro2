...
Public surface:
    ...
    update_project_for_soc_pro2(sdk_root, soc, flash_ntz, build_dir, cmake_project_dir)  (auto-mode incremental)
...
def update_project_for_soc_pro2(
    sdk_root: str,
    soc: str,
    flash_ntz: str,
    build_dir: str,
    ...
    """Called by build_firmware on success (Pro2 variant).

    Pro2 produces a single flat flash_ntz.bin — there is no multi-image
    flash layout header to parse, so this function takes the build outputs
    directly instead of a ParsedProject.

    Behaviour:
      - File missing → create from EMPTY_PROJECT_INFO, then add this SoC
        as `auto`.
      - SoC entry missing → add as `auto` with the flash_ntz image.
      - SoC entry exists and is `auto` → overwrite build_dir + images
        (the "generated" region).
      - SoC entry exists and is `manual` with the same build_dir → update
        images only (stale image path from a previous build, e.g.
        flash_ntz.nn.bin → flash_ntz.bin after switching away from NN).
        The mode stays `manual` so any user-customised fields are preserved.
      - SoC entry exists and is `manual` with a different build_dir →
        leave untouched (user owns it; different build_dir means it really
        is a hand-crafted flash layout, not a stale MCP-generated entry).
    ...
    existing = info.projects.get(soc)
    if existing is None or existing.flash_layout_setting_mode == "auto":
        info.projects[soc] = ProjectEntry(
            flash_layout_setting_mode="auto",
            build_dir=build_dir,
            images=images,
        )
    elif os.path.normcase(existing.build_dir) == os.path.normcase(build_dir):
        # manual mode but same build_dir: stale image path (e.g. after
        # switching between NN and non-NN build). Update images only —
        # keep mode and any other user fields intact.
        existing.images = images
    # manual + different build_dir: user-managed, leave untouched
...
def masked_board_info_dict(info: BoardInfo) -> dict:
    """Serialise BoardInfo for MCP resource exposure.

    Pro2 BoardEntry has no password fields (no remote transport), so no
    masking is applied.  _PASSWORD_MASK is kept as a placeholder — if a
    password field is added in the future, apply it here (cf. ameba-rtos
    implementation which masks remote.password).
    """
