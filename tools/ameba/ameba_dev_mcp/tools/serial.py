...
def serial_connect(alias: Optional[str] = None, *,
    ...
    (default), pulses DTR/RTS right after the port is open.
    If `wait_for` is non-empty, blocks until any pattern matches OR a stop
    condition fires.
...
def register_serial_tools(mcp: FastMCP) -> None:
    ...
    async def serial_connect_tool(
        ...
        session is already open, the existing one is reused.
        ...
        Args:
            ...
            reset:        Pulse DTR/RTS right after the port is open.
                          Default True — matches the common "open monitor +
                          reboot to capture fresh boot log" flow. Set to
                          False when attaching to an already-running board
                          to read ongoing logs without rebooting.
