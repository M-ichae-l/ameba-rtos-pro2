...
/* RAM disk configuration (used when CONFIG_USBD_MSC_SD_MODE is not defined) */
#ifndef CONFIG_USBD_MSC_SD_MODE
...
typedef struct {
	...
	usb_os_sema_t rx_sema;                          /**< Semaphore to signal data reception. */
	rtos_task_t tx_task;                            /**< RTOS task handle for data transmission. */
	usb_os_sema_t tx_sema;                          /**< Semaphore to signal data transmission. */
 ...
 * @return 0 on success, non-zero on failure.
 */
/* Renamed via macro: usb_class/device/class/msc/src/usbd_msc.c is built
 * unconditionally into libusbd for every usb_stack and also exports
 * usbd_msc_init/usbd_msc_deinit (different signature: takes N_bh/Size_bh/
 * msc_opts instead of a usbd_msc_cb_t), causing a link-time multiple
 * definition. Same fix as usbd_cdc_acm_init in
 * component/usb/usb_stack/v1.0.0/device/cdc_acm/usbd_cdc_acm.h: substitute this file's
 * own definitions and every caller that includes this header, with no other
 * files touched. */
#define usbd_msc_init usbd_msc_pro3_init
#define usbd_msc_deinit usbd_msc_pro3_deinit
