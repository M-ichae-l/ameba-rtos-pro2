...
/* static: component/usb/usb_class/device/class/uvc/usbd_uvc.c (unconditionally
   compiled) also defines non-static globals named p_data/x_data - keeping
   these static avoids the link collision without needing a rename macro
...
static int usbd_uvc_send_response(usbd_uvc_dev_t *uvc, usbd_uvc_request_data_t *data)
{
	...
	if (data->length > 0) {
		usb_os_memcpy(ep0_in->xfer_buf, data->data, (u32)data->length);
...
void usbd_uvc_get_command_process_unit(usbd_uvc_dev_t *dev, usb_setup_req_t *ctrl, usbd_uvc_request_data_t *resp)
{
	if (dev->command_entity == UVC_VC_PROCESS_UNIT) {
		...
		case USBD_UVC_GET_CUR:
			usb_os_memcpy(resp->data, (const void *)&p_data.cur, (u32)ctrl->wLength);
			break;

		case USBD_UVC_GET_MIN:
			usb_os_memcpy(resp->data, (const void *)&p_data.min, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_MAX:
			usb_os_memcpy(resp->data, (const void *)&p_data.max, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_DEF:
			usb_os_memcpy(resp->data, (const void *)&p_data.def, (u32)ctrl->wLength);
			break;

		case USBD_UVC_GET_RES:
			usb_os_memcpy(resp->data, (const void *)&p_data.res, (u32)ctrl->wLength);
			break;

		case USBD_UVC_GET_LEN:
			usb_os_memcpy(resp->data, (const void *)&p_data.len, (u32)ctrl->wLength);
			break;

		case USBD_UVC_GET_INFO:
			usb_os_memcpy(resp->data, (const void *)&p_data.info, (u32)ctrl->wLength);
			break;
		}
		...
		case USBD_UVC_SET_CUR:
			break;
		case USBD_UVC_GET_CUR:
			usb_os_memcpy(resp->data, (const void *)&x_data.cur, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_MIN:
			usb_os_memcpy(resp->data, (const void *)&x_data.min, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_MAX:
			usb_os_memcpy(resp->data, (const void *)&x_data.max, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_DEF:
			usb_os_memcpy(resp->data, (const void *)&x_data.def, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_RES:
			usb_os_memcpy(resp->data, (const void *)&x_data.res, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_LEN:
			usb_os_memcpy(resp->data, (const void *)&x_data.len, (u32)ctrl->wLength);
			break;
		case USBD_UVC_GET_INFO:
			usb_os_memcpy(resp->data, (const void *)&x_data.info, (u32)ctrl->wLength);
			break;
		}
	}
}
...
__weak void get_command_extension_unit(usbd_uvc_dev_t *dev, usb_setup_req_t *ctrl, usbd_uvc_request_data_t *resp)
{
	...
	case USBD_UVC_SET_CUR:
		break;
	case USBD_UVC_GET_CUR:
		usb_os_memcpy(resp->data, (const void *)&x_data.cur, (u32)sizeof(int));
		break;
	case USBD_UVC_GET_MIN:
		usb_os_memcpy(resp->data, (const void *)&x_data.min, (u32)sizeof(int));
		break;
	case USBD_UVC_GET_MAX:
		usb_os_memcpy(resp->data, (const void *)&x_data.max, (u32)sizeof(int));
		break;
	case USBD_UVC_GET_DEF:
		usb_os_memcpy(resp->data, (const void *)&x_data.def, (u32)sizeof(int));
		break;
	case USBD_UVC_GET_RES:
		usb_os_memcpy(resp->data, (const void *)&x_data.res, (u32)sizeof(int));
		break;
	case USBD_UVC_GET_LEN:
		usb_os_memcpy(resp->data, (const void *)&x_data.len, (u32)sizeof(int));
		break;
	case USBD_UVC_GET_INFO:
		usb_os_memcpy(resp->data, (const void *)&x_data.info, (u32)sizeof(int));
		break;
	}
}
								  ...
								  usbd_uvc_request_data_t *resp)
{
	...
	case USBD_UVC_GET_CUR:
		if (cs == USBD_UVC_VS_PROBE_CONTROL) {
			usb_os_memcpy(ctrl_stream, &dev->probe, (u32)sizeof(*ctrl_stream));
			RTK_LOGS(TAG, RTK_LOG_INFO, "Probe\r\n");
		} else {
			usb_os_memcpy(ctrl_stream, &dev->commit, (u32)sizeof(*ctrl_stream));
...
usbd_uvc_streaming_process_data(usbd_uvc_dev_t *dev, usbd_uvc_request_data_t *data)
{
	...
	if (iformat < 1U || iformat > uvcd_nformats) {
		RTK_LOGS(TAG, RTK_LOG_WARN, "Invalid bFormatIndex %u\r\n", iformat);
		return;
	}

	format = &uvcd_formats[iformat - 1];
	if (format->fcc != USBD_UVC_FORMAT_TYPE_H264) {
		RTK_LOGS(TAG, RTK_LOG_WARN,
				 "Host selected format %u (fcc=%d); this device only streams H.264 1080p 30fps - please reselect the H.264 format\r\n",
				 iformat, format->fcc);
	}
	nframes = 0;
	while (format->frames[nframes].width != 0) {
	...
	if (iframe < 1U || iframe > nframes) {
		RTK_LOGS(TAG, RTK_LOG_WARN, "Invalid bFrameIndex %u\r\n", iframe);
		return;
	}
