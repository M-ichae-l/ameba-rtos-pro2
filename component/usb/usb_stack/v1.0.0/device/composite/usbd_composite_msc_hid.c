...
static const u8 usbd_composite_dev_desc[USB_LEN_DEV_DESC] = {
	...
	0x00,															/* bDeviceProtocol: per-interface (no IAD needed) */
...
/* USB MSC + HID Device High Speed Configuration Descriptor */
...
/**
  * @brief  Set HID+MSC class configuration
...
/**
  * @brief  Clear HID+MSC configuration
  * @note   This function is called within an interrupt service routine (ISR) context;
  *         time-consuming operations (e.g., `malloc`, `rtos_sema_take`) are not permitted.
  * @param  dev: USB device instance
  * @param  config: USB configuration index
...
/**
  * @brief  Handle HID/MSC specific CTRL requests
  * @note   This function is called within an interrupt service routine (ISR) context;
  *         time-consuming operations (e.g., `malloc`, `rtos_sema_take`) are not permitted.
  * @param  dev: USB device instance
	...
	case USB_REQ_TYPE_STANDARD:
		...
		default:
			if (req->wIndex == USBD_COMP_HID_ITF) {
				/* Get HID/report descriptor (USB_REQ_GET_DESCRIPTOR is a
				 * Standard request when the HID/report descriptor is
				 * targeted at an interface - composite_hid_setup() handles
				 * it, see usbd_composite_hid.c). */
...
static int usbd_composite_handle_ep_data_in(usb_dev_t *dev, u8 ep_addr, u8 status)
{
	...
	if (ep_addr == USBD_COMP_HID_INTR_IN_EP) {
...
static int usbd_composite_handle_ep_data_out(usb_dev_t *dev, u8 ep_addr, u32 len)
{
	...
	if (ep_addr == USBD_COMP_MSC_BULK_OUT_EP) {
...
/**
  * @brief  Init MSC+HID composite class
  * @param  hid_tx_buf_len: HID interrupt IN xfer buffer size
...
int usbd_composite_msc_hid_init(u16 hid_tx_buf_len, const usbd_composite_hid_usr_cb_t *hid_cb, const usbd_composite_cb_t *cb)
{
	...
	if ((hid_cb == NULL) || (cb == NULL)) {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "Invalid user CB\n");
		return HAL_ERR_PARA;
	}
	cdev->cb = cb;

	ret = usbd_composite_hid_init(cdev, hid_tx_buf_len, hid_cb);
	if (ret != HAL_OK) {
...
/**
  * @brief  DeInit MSC+HID composite class
  * @param  void
  * @retval Status
  */
