...
/* Add lock to avoid msc tx_thread preempts msc rx_thread when read the disk */
...
{
	return usb_sd_deinit();
}

#else /* !CONFIG_USBD_COMPOSITE_MSC_SD_MODE */
...
#endif /* CONFIG_USBD_COMPOSITE_MSC_SD_MODE */
...
static void usbd_composite_msc_rx_thread(void *param)
{
	...
	for (;;) {
		if (usb_os_sema_take(mdev->rx_sema, USB_OS_SEMA_TIMEOUT) == HAL_OK) {
...
static void usbd_composite_msc_tx_thread(void *param)
{
	...
	for (;;) {
		if (usb_os_sema_take(mdev->tx_sema, USB_OS_SEMA_TIMEOUT) == HAL_OK) {
...
int usbd_composite_msc_init(usbd_composite_dev_t *cdev)
{
...
#ifdef CONFIG_USBD_COMPOSITE_MSC_SD_MODE
	ops->disk_getcapacity = usb_sd_getcapacity;
	ops->disk_read = usb_sd_readblocks;
	ops->disk_write = usb_sd_writeblocks;
#else
	...
	ret = usb_os_sema_create(&mdev->rx_sema);
	if (ret != HAL_OK) {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "Create RX sema fail\n");
		goto create_rx_sema_fail;
	}

	ret = usb_os_sema_create(&mdev->tx_sema);
	if (ret != HAL_OK) {
...
create_rx_thread_fail:
	usb_os_sema_delete(mdev->tx_sema);

create_tx_sema_fail:
	usb_os_sema_delete(mdev->rx_sema);
