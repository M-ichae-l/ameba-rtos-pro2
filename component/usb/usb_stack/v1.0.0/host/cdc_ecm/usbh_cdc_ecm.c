...
u8 usbh_cdc_ecm_prepare_done(void)
{
	...
	/* For 4G dongles (non-RTK VID) the data path is confirmed by the AT
	 * command layer (AT+QNETDEVCTL / AT+NETOPEN / …) before prepare_done()
	 * is called.  The CDC NOTIFY_NETWORK_CONNECTION mechanism is unreliable
	 * in host-reset scenarios: a COPS de-registration sends a "link-down"
	 * notification that fills the device's single-entry interrupt FIFO,
	 * causing the subsequent "link-up" notification to be silently dropped.
	 * usbh_cdc_ecm_sof() gates bulk TX/RX on eth_hw_connect, so a missed
	 * "link-up" notification permanently prevents data from flowing.
	 *
	 * usbh_cdc_ecm_get_link_status() already returns 1 unconditionally for
	 * non-RTK VIDs; apply the same logic here so the SOF bulk gate is
	 * consistent with the link-status API. */
	if (cdc->vid != USB_VID) {
		cdc->eth_hw_connect = 1;
	}
