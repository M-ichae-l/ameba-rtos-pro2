...
static int usbh_msc_attach(usb_host_t *host)
{
	...
	if (itf_data == NULL) {
		 ...
		 * pipe_num == 0 is the existing class-driver unopened-pipe sentinel:
		 * before class attach(), the host core has allocated control OUT/IN
		 * pipes during enumeration, occupying pipe 0 and pipe 1. Therefore
		 * MSC bulk pipes are allocated from pipe 2 onward in this lifecycle. */
