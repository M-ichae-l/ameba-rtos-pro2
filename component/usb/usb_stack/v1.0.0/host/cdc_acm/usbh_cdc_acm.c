...
#if CONFIG_USBH_CDC_ACM_4G_DONGLE
	{
		/* Some 4G dongles (e.g. Quectel EG91/EG915) expose their AT command
		 ...
		 *
		 * Scoped to known dongle VIDs (unlike a bare class/subclass match) so
		 * Phase-1 does not also claim an unrelated device's vendor-specific
		 * interface - e.g. some Quectel Unisoc-platform modules (EG915U)
		 * briefly enumerate with the chipset vendor's own VID/PID (e.g.
		 * Spreadtrum 0x1782) before resetting and re-enumerating with the
		 * Quectel VID/PID; without the VID match, that transient device's
		 * attach()/rollback churn could interfere with catching the real
		 * re-enumeration that follows. */
		.mMatchFlags = USBH_DEV_ID_MATCH_VID | USBH_DEV_ID_MATCH_ITF_CLASS | USBH_DEV_ID_MATCH_ITF_SUBCLASS,
		.idVendor = USBH_CDC_ACM_QUECTEL_DONGLE_VID,
		.bInterfaceClass = 0xFF,
		.bInterfaceSubClass = 0x00,
	},
	{
		.mMatchFlags = USBH_DEV_ID_MATCH_VID | USBH_DEV_ID_MATCH_ITF_CLASS | USBH_DEV_ID_MATCH_ITF_SUBCLASS,
		.idVendor = USBH_CDC_ACM_FIBOCOM_DONGLE_LE271_VID,
		.bInterfaceClass = 0xFF,
		.bInterfaceSubClass = 0x00,
	},
	{
		.mMatchFlags = USBH_DEV_ID_MATCH_VID | USBH_DEV_ID_MATCH_ITF_CLASS | USBH_DEV_ID_MATCH_ITF_SUBCLASS,
		.idVendor = USBH_CDC_ACM_SIMCOM_DONGLE_SIM767X_VID,
		.bInterfaceClass = 0xFF,
		.bInterfaceSubClass = 0x00,
	},
#endif
...
static int usbh_cdc_acm_attach(usb_host_t *host)
{
	...
	/* Quectel: the single vendor-specific interface already carries all of its
	 * endpoints at alternate setting 0 (usbh_cdc_acm_attach_quectel() never
	 * finds/uses an alt=1) - there is no alternate setting to switch into, so
	 * a SET_INTERFACE(itf, 0) here is a no-op on the wire but has been observed
	 * to make some modules (e.g. Quectel EG915U) drop off the bus while still
	 * completing enumeration. Skip straight to line coding for this path; the
	 * standard ACM 4G topology (data itf alt=0/1) still needs it. */
	cdc->sub_status = (vid == USBH_CDC_ACM_QUECTEL_DONGLE_VID) ?
					  USBH_CDC_ACM_CTRL_SET_LINE_CODING : USBH_CDC_ACM_CTRL_ALT_SETTING;
#else
	usbh_itf_data_t *itf_data;
