...
volatile uint8_t ota_wifi_cancel_flag;
...
{
    if (device_id == 1) {
        ota_wifi_cancel_flag = 1;
        printf("[OTA] Wifi cancellation invoked\n");
        return ota_wifi_cancel_flag;
    }
...
int ext_storage_update_ota(char *filename)
{
    ...
    while (idx < file_size) {
        loop_counter++;
        if (ota_wifi_cancel_flag) {
            printf("\n\r[%s] OTA upgrade canceled by user\n\r", __FUNCTION__);
            loop_counter = 0;
            ret = -2;
            ota_clear_cancel_status(1);
            goto update_ota_exit;
        }
...
int heap_update_ota(uint8_t *buffer, uint32_t length)
{
    ...
    if (!buffer || length < 0x2D8) {
        printf("\n\r[%s] Invalid buffer or length too small for timestamp read\n\r", __FUNCTION__);
        ret = -5;
        goto update_ota_exit;
    }
    ...
    uint32_t ota_timestamp = ((uint32_t)buffer[0x2D4]) | ((uint32_t)buffer[0x2D5] << 8) | ((uint32_t)buffer[0x2D6] << 16) | ((uint32_t)buffer[0x2D7] << 24);
    ...
    if (fw_timest > ota_timestamp) {
        printf("\n\r[%s] OTA firmware is older than current, skip update\n\r", __FUNCTION__);
        ret = -4;
        goto update_ota_exit;
    }
