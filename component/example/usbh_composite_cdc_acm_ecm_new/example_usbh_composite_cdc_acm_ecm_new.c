...
/*
 * The Quectel/Fibocom/SIMCom AT-command dial state machines are retained, with
 * two classes of SDK adaptation:
 *
 *  1. component/os/os_wrapper does not exist in this SDK generation (same gap
 *     as example_usbd_cdc_acm_new.c / component/usb/usb_stack/<version>/hal/ameba_soc.h) -
 *     rtos_sema_t/rtos_sema_create/take/give/delete are mapped onto this
 *     project's usb_os_* (already FreeRTOS-backed, see
 *     component/usb/usb_stack/<version>/common/usb_os.c) via the macros below instead of
 *     rewriting every call site; rtos_task_xxx, RTK_SUCCESS, RTK_LOGS and
 *     CACHE_LINE_SIZE already come from component/usb/usb_stack/<version>/hal/ameba_soc.h
 *     transitively.
 *
 *  2. This project has no lwip<->USB-Ethernet netif bridge for the ported
 *     driver. component/ethernet_mii/ethernet_usb.c is this project's own
 *     (currently dormant, CONFIG_ETHERNET==0 by default) lwip<->USB-Ethernet
 *     bridge for the *legacy* host_new/cdc_ecm driver - same netif plumbing
 *     (xnetif[], NET_IF_NUM, LwIP_Init/LwIP_DHCP/LwIP_ReleaseIP,
 *     ethernetif_mii_init's rltk_mii_send/rltk_mii_recv driver-callback
 *     contract), just pointed at usbh_cdc_ecm_senddata() (legacy API) instead
 *     of usbh_cdc_ecm_send_data() (the ported driver's API). Since the two USB
 *     stacks' cdc_ecm APIs cannot both be linked into one image (see
 *     libusbd.cmake's usb_stack switch), ethernet_usb.c's build is now gated
 *     to !usb_stack-is-new the same way, and the netif bridge functions
 *     (rltk_mii_send/rltk_mii_recv/usbh_comp_ecm_netif_up/down below) are
 *     this port's own equivalent, calling this driver's cdc_ecm API.
 */
...
#define rtos_sema_create(sema, init_count, max_count) usb_os_sema_create(sema)
#define rtos_sema_delete(sema)                        usb_os_sema_delete(sema)
#define rtos_sema_take(sema, timeout_ms)              usb_os_sema_take((sema), (timeout_ms))
#define rtos_sema_give(sema)                          usb_os_sema_give(sema)
#ifndef RTOS_SEMA_MAX_COUNT
#define RTOS_SEMA_MAX_COUNT                           USB_OS_SEMA_TIMEOUT
#endif
typedef usb_os_sema_t rtos_sema_t;
...
/* usbh_comp_dongle_diag_cmd() return values.
 * 0                              - success, ECM data ready.
 * DONGLE_DIAG_RET_REBOOT_PENDING - dongle needs a full USB detach/reattach
 *                                  before it will behave correctly (e.g. some
 *                                  modules only apply a re-issued AT+CGDCONT
 *                                  after a fresh enumeration); usbh_comp_do_init()
 *                                  retries the whole bring-up, bounded by
 *                                  CONFIG_USBH_COMP_REBOOT_PENDING_MAX_RETRY.
 * any other non-zero              - fatal AT error, give up.
 */
#define DONGLE_DIAG_RET_REBOOT_PENDING                          2
#define CONFIG_USBH_COMP_REBOOT_PENDING_MAX_RETRY               3
/* Private types -------------------------------------------------------------*/
/* Ethernet State */
typedef enum {
	ETH_STATUS_IDLE = 0U,
...
typedef enum {
	QUECTEL_DONGLE_STATUS_RESERVED = 0U, /* was IDLE (debug-only, removed) */
	QUECTEL_DONGLE_STATUS_ECM_CFG,
	QUECTEL_DONGLE_STATUS_SEARCH,        /* unused by EG915; used by EG91 */
	QUECTEL_DONGLE_STATUS_SET_APN,
	QUECTEL_DONGLE_STATUS_DIAG,
	QUECTEL_DONGLE_STATUS_DIAG_STATUS,
	QUECTEL_DONGLE_STATUS_GET_MAC,       /* unused by EG915; used by EG91 */
	QUECTEL_DONGLE_STATUS_WAIT_REG,
	QUECTEL_DONGLE_STATUS_MAX,
	QUECTEL_DONGLE_STATUS_QUICK_CHECK,   /* initial state; host-reset fast-path */
...
static const usbh_config_t usbh_cfg = {
	...
	/* 4U matches the validated priority used by component/example/usbd_cdc_acm_new. */
	.isr_priority = 4U,
	.main_task_stack_size = 1792U,
	.main_task_priority = CONFIG_USBH_COMP_INIT_THREAD_PRIORITY,
	.tick_source = USBH_SOF_TICK,
	.class_num = 2U,   /* CDC-ACM + CDC-ECM */
	/* amebapro2 (rtl8735b) FIFO total depth is 2232 DWORD, with 8 DWORD
	 * reserved for DMA addresses. */
	.rx_fifo_depth = 1712,
	.nptx_fifo_depth = 256,
	.ptx_fifo_depth = 256,
};
...
static void usbh_comp_dongle_set_netinfo(u8 *pbuf, u8 *name)
{
	 ...
	 * Always NUL-terminate after memcpy: a second write with a shorter
	 * string would otherwise leave stale bytes from the previous value
	 * (e.g. "10.71.147.172" written over "10.120.118.192" produces
	 * "10.71.147.1722" because position 13 is not overwritten). */
	if (strcmp(pname, "gw") == 0) {
...
static int usbh_comp_acm_rxdata(u8 *pbuf, u32 len, u8 status)  /* type is usb transfer type */
{
	...
	if (USBH_CDC_ACM_QUECTEL_DONGLE_VID == vid) {
		...
		/* +CGDCONT: <cid>,<PDP_type>,<APN>,... - confirm the APN we requested
		 * with AT+CGDCONT= was actually accepted, per Quectel's documented
		 * flow ("check the setting" with AT+CGDCONT?), rather than assuming
		 * the "OK" from the set command meant it took. */
		if (strstr((char *)pbuf, "+CGDCONT:") && strstr((char *)pbuf, "\"" APN_NAME "\"")) {
			usbh_dongle_ctx.quectel.apn_ready = 1;
		}
		/* +CGATT: <state> - state==1 means attached to the packet-data service.
		 * Per Quectel's documented flow, CEREG and CGATT should both be ready
		 * before AT+QNETDEVCTL is issued. */
		if (strstr((char *)pbuf, "+CGATT:")) {
			...
			if (*p == '1') {
				usbh_dongle_ctx.quectel.cgatt_ready = 1;
			}
		}
		/* +QNETDEVSTATUS: <status> - status==1 means the QNETDEVCTL-activated
		 * network device is actually connected (the "OK" from AT+QNETDEVCTL
		 * only means the async activation was accepted, not that it finished). */
		if (strstr((char *)pbuf, "+QNETDEVSTATUS:")) {
			char *p = strstr((char *)pbuf, "+QNETDEVSTATUS:") + sizeof("+QNETDEVSTATUS:") - 1;
			while (*p == ' ') {
				p++;
			}
			if (*p == '1') {
				usbh_dongle_ctx.quectel.netdev_ready = 1;
			}
		}
...
static u8 usbh_comp_quectel_eg915_ctrl(void)
{
	...
	u8 apn_retry = 0;
	u8 wait_reg_retry = 0;
	u8 diag_status_retry = 0;
	quectel_dongle_state_t state = QUECTEL_DONGLE_STATUS_QUICK_CHECK;

	u8 *pdata1 = (u8 *)"AT+QCFG=\"usbnet\",1";
	u8 *pdata2 = (u8 *)"AT+QNWINFO;+CEREG?;+CREG?;+QCSQ;+CSQ;+COPS?;+CPIN?;+QCCID;+CGATT?";
	/* Query-only check, tried first: if the context already carries our APN
	 * (e.g. left over from a previous session), skip AT+CGDCONT= entirely -
	 * re-issuing it against an already-matching/active context has been
	 * observed to require a full USB detach/reattach before the module
	 * reports correctly again. */
	u8 *pdata3q = (u8 *)"AT+CGDCONT?";
	/* Only reached when the query above did not already confirm the APN: set
	 * it then immediately read it back in the same compound command, so
	 * usbh_comp_acm_rxdata() sees the +CGDCONT: confirmation line without a
	 * separate round trip - per Quectel's documented flow ("check the
	 * setting" with AT+CGDCONT?). */
	u8 *pdata3 = (u8 *)"AT+CGDCONT=1,\"IP\",\"" APN_NAME "\";+CGDCONT?";
	u8 *pdata4 = (u8 *)"AT+QNETDEVCTL=1,1,1";
	u8 *pdata5 = (u8 *)"AT+CGCONTRDP";
	/* Combined quick-check query: CEREG for registration, CGATT for PS attach,
	 * CGCONTRDP for active PDP context / IP.  All three are already parsed by
	 * usbh_comp_acm_rxdata() into cereg_ready / cgatt_ready / ip_ready. */
	u8 *pdata_qc = (u8 *)"AT+CEREG?;+CGATT?;+CGCONTRDP";
	/* Network de/re-registration cycle used when a stale WWAN activation is
	 * detected after a host reset.  AT+COPS=2 forces the module to deregister
	 * from the LTE network; AT+COPS=0 re-enables automatic network selection.
	 * This resets the modem's internal PS state so the subsequent
	 * AT+QNETDEVCTL=1,1,1 in DIAG produces the expected +QNETDEVSTATUS:1 URC
	 * and the ECM data path is properly activated. */
	u8 *pdata_cops_dereg = (u8 *)"AT+COPS=2";
	u8 *pdata_cops_auto  = (u8 *)"AT+COPS=0";
	...
	for (;;) {
		...
		switch (state) {
		case QUECTEL_DONGLE_STATUS_QUICK_CHECK:
			/* Host-reset fast-path.  The dongle is already in ECM mode
			 * (confirmed by VID/PID 0x2C7C/0x0901) and may still hold an
			 * active LTE session.  Send a combined query and wait up to 10 s
			 * for the readiness flags set by usbh_comp_acm_rxdata() before
			 * deciding which init path to take:
			 *   cereg+cgatt (±ip_ready) → skip ECM-cfg/APN/reg, go to DIAG
			 *                             (AT+QNETDEVCTL must always be sent to
			 *                             re-activate the modem ECM data path
			 *                             after USB re-enumeration; if ip_ready
			 *                             is already set, DIAG_STATUS is a no-op)
			 *   else                    → full init from ECM_CFG */
			RTK_LOGS(TAG, RTK_LOG_INFO, "QUICK_CHECK: probing existing connection\n");
			memset(usbh_comp_acm_tx_buf, 0x00, CONFIG_USBH_COMP_PBUF_MAX_LEN);
			memcpy(usbh_comp_acm_tx_buf, pdata_qc, usbh_comp_strlen(pdata_qc));
			usbh_comp_acm_tx_buf[usbh_comp_strlen(pdata_qc) + 0] = 0x0D;
			usbh_comp_acm_tx_buf[usbh_comp_strlen(pdata_qc) + 1] = 0x0A;
			if (HAL_OK != usbh_comp_acm_transmit(usbh_comp_acm_tx_buf, usbh_comp_strlen(usbh_comp_acm_tx_buf))) {
				RTK_LOGS(TAG, RTK_LOG_WARN, "QUICK_CHECK: transmit failed, fallback to full init\n");
				state = QUECTEL_DONGLE_STATUS_ECM_CFG;
				break;
			}
			for (idx = 0; idx < 10 && usbh_dongle_ctx.in_detach == 0; idx++) {
			...
			if ((usbh_dongle_ctx.quectel.cereg_ready != 0) &&
				(usbh_dongle_ctx.quectel.cgatt_ready != 0)) {
				/* EG915U host-reset stale-WWAN recovery.
				 *
				 * After a host CPU reset with the dongle powered, the modem
				 * keeps its LTE registration (cereg+cgatt still valid) but
				 * the WWAN data channel is in an inconsistent state:
				 *  - AT+QNETDEVCTL=1,0,0 → CME ERROR:53 (refused)
				 *  - AT+QNETDEVCTL=1,1,1 → OK but no +QNETDEVSTATUS:1 URC
				 *  - USB stack teardown (REBOOT_PENDING) does NOT cause the
				 *    modem to deregister; re-enum always comes back cereg=1
				 *    and the same stale state repeats.
				 *
				 * Root fix: force a full PS de/re-registration via AT+COPS so
				 * the modem's internal WWAN state is reset from scratch.
				 * After COPS=2 (deregister) + COPS=0 (auto re-register),
				 * AT+QNETDEVCTL=1,1,1 reliably produces +QNETDEVSTATUS:1
				 * and the ECM data path activates normally.
				 *
				 * Skip ECM-cfg (already ECM mode) and APN (unchanged);
				 * go to WAIT_REG to confirm fresh cereg+cgatt, then DIAG. */
				RTK_LOGS(TAG, RTK_LOG_INFO,
						 "QUICK_CHECK: registered%s — stale WWAN, forcing PS re-registration\n",
						 usbh_dongle_ctx.quectel.ip_ready ? "+ip" : "");

				/* Step 1: deregister from network. */
				memset(usbh_comp_acm_tx_buf, 0x00, CONFIG_USBH_COMP_PBUF_MAX_LEN);
				memcpy(usbh_comp_acm_tx_buf, pdata_cops_dereg, usbh_comp_strlen(pdata_cops_dereg));
				usbh_comp_acm_tx_buf[usbh_comp_strlen(pdata_cops_dereg) + 0] = 0x0D;
				usbh_comp_acm_tx_buf[usbh_comp_strlen(pdata_cops_dereg) + 1] = 0x0A;
				usbh_comp_acm_transmit(usbh_comp_acm_tx_buf, usbh_comp_strlen(usbh_comp_acm_tx_buf));
				/* Allow ~3 s for the module to complete deregistration. */
				for (idx = 0; idx < 30 && usbh_dongle_ctx.in_detach == 0; idx++) {
				...
				/* Step 2: re-enable automatic network selection. */
				memset(usbh_comp_acm_tx_buf, 0x00, CONFIG_USBH_COMP_PBUF_MAX_LEN);
				memcpy(usbh_comp_acm_tx_buf, pdata_cops_auto, usbh_comp_strlen(pdata_cops_auto));
				usbh_comp_acm_tx_buf[usbh_comp_strlen(pdata_cops_auto) + 0] = 0x0D;
				usbh_comp_acm_tx_buf[usbh_comp_strlen(pdata_cops_auto) + 1] = 0x0A;
				usbh_comp_acm_transmit(usbh_comp_acm_tx_buf, usbh_comp_strlen(usbh_comp_acm_tx_buf));

				/* Clear all stale flags; WAIT_REG will detect fresh
				 * registration and DIAG will activate the bearer. */
				usbh_dongle_ctx.quectel.cereg_ready  = 0;
				usbh_dongle_ctx.quectel.cgatt_ready  = 0;
				usbh_dongle_ctx.quectel.ip_ready     = 0;
				usbh_dongle_ctx.quectel.netdev_ready = 0;
				state = QUECTEL_DONGLE_STATUS_WAIT_REG;
			} else {
				/* No active connection found: run the full initialisation. */
				RTK_LOGS(TAG, RTK_LOG_INFO, "QUICK_CHECK: no connection, full init\n");
				state = QUECTEL_DONGLE_STATUS_ECM_CFG;
			}
			break;
		case QUECTEL_DONGLE_STATUS_ECM_CFG: {
			...
			/* Per Quectel's documented flow, the APN (CGDCONT) must be configured
			 * before the module attempts to register/attach - the EPS attach
			 * procedure can piggy-back the default PDN/APN request, so setting it
			 * afterwards left CEREG/CGATT stuck forever on some networks. Go
			 * straight to SET_APN here instead of SEARCH. */
		state = QUECTEL_DONGLE_STATUS_SET_APN;
		break;
		case QUECTEL_DONGLE_STATUS_SET_APN: {
			u8 *apn_cmd;
			/* Don't just trust the "OK" from AT+CGDCONT= - confirm with
			 * AT+CGDCONT? (parsed into apn_ready by usbh_comp_acm_rxdata())
			 * per Quectel's documented flow, retrying a bounded number of
			 * times before giving up. First attempt is query-only: if the
			 * context already matches, never touch AT+CGDCONT= at all. */
			if (usbh_dongle_ctx.quectel.apn_ready != 0) {
				state = QUECTEL_DONGLE_STATUS_WAIT_REG;
				break;
			}
			if (apn_retry >= 5) {
				/* Some modules only apply a re-issued AT+CGDCONT after a fresh
				 * USB enumeration - ask the caller to detach/reattach and
				 * retry the whole bring-up instead of giving up outright. */
				RTK_LOGS(TAG, RTK_LOG_ERROR, "APN not confirmed after retries, request reconnect\n");
				return DONGLE_DIAG_RET_REBOOT_PENDING;
			}
			apn_cmd = (apn_retry == 0) ? pdata3q : pdata3;
			apn_retry++;
			memset(usbh_comp_acm_tx_buf, 0x00, CONFIG_USBH_COMP_PBUF_MAX_LEN);
			memcpy(usbh_comp_acm_tx_buf, apn_cmd, usbh_comp_strlen(apn_cmd));
			usbh_comp_acm_tx_buf[usbh_comp_strlen(apn_cmd) + 0] = 0x0D;
			usbh_comp_acm_tx_buf[usbh_comp_strlen(apn_cmd) + 1] = 0x0A;
			if (HAL_OK != usbh_comp_acm_transmit(usbh_comp_acm_tx_buf, usbh_comp_strlen(usbh_comp_acm_tx_buf))) {
				break;
			}
			for (idx = 0; idx < 10 && usbh_dongle_ctx.in_detach == 0; idx++) {
		...
		break;
		case QUECTEL_DONGLE_STATUS_WAIT_REG:
			/* APN is already set (SET_APN ran first); this state's job is only
			 * to wait for CEREG (registered) + CGATT (attached), re-querying
			 * (absorbs what used to be the separate SEARCH state) until both
			 * are ready or we give up. Do not start AT+QNETDEVCTL any earlier -
			 * racing it ahead of registration just leaves AT+CGCONTRDP polling
			 * forever with no PDP context to report. */
			if ((usbh_dongle_ctx.quectel.cereg_ready != 0) && (usbh_dongle_ctx.quectel.cgatt_ready != 0)) {
				state = QUECTEL_DONGLE_STATUS_DIAG;
				break;
			}
			if (wait_reg_retry >= 20) {
				RTK_LOGS(TAG, RTK_LOG_ERROR, "Not registered/attached after retries, abort\n");
				return 1;
			}
			wait_reg_retry++;
			memset(usbh_comp_acm_tx_buf, 0x00, CONFIG_USBH_COMP_PBUF_MAX_LEN);
		...
		case QUECTEL_DONGLE_STATUS_DIAG: {
			...
			/* AT+QNETDEVCTL's "OK" only means the async activation request was
			 * accepted, not that the network device is actually up - wait for the
			 * +QNETDEVSTATUS: 1 URC (bounded to the same 15s budget as before),
			 * exiting early once it arrives instead of always sleeping the full
			 * window. */
			for (idx = 0; idx < 150 && usbh_dongle_ctx.in_detach == 0; idx++) {
				if (usbh_dongle_ctx.quectel.netdev_ready != 0) {
					break;
				}
				rtos_time_delay_ms(100);
			}
			...
			if (0 == usbh_dongle_ctx.quectel.netdev_ready) {
				RTK_LOGS(TAG, RTK_LOG_WARN, "QNETDEVCTL not confirmed by QNETDEVSTATUS yet\n");
			}
		}
		state = QUECTEL_DONGLE_STATUS_DIAG_STATUS;
		break;
		case QUECTEL_DONGLE_STATUS_DIAG_STATUS:
			if (0 == usbh_dongle_ctx.quectel.ip_ready) {
				/* No PDP context ever became active - give up instead of
				 * silently re-sending AT+CGCONTRDP forever (see analysis: this
				 * happens when the module never finishes network registration,
				 * e.g. weak signal). */
				if (diag_status_retry >= 20) {
					RTK_LOGS(TAG, RTK_LOG_ERROR, "No IP after retries, network not attached, abort\n");
					return 1;
				}
				diag_status_retry++;
				memset(usbh_comp_acm_tx_buf, 0x00, CONFIG_USBH_COMP_PBUF_MAX_LEN);
...
static int usbh_comp_dongle_netif_init(void)
{
	...
	if (NULL == pdnsserver) {
		...
		/* dns_setserver() takes ip_addr_t* (a tagged union w/ LWIP_IPV6=1,
		 * bigger than plain ip4_addr_t) - passing &dnsserver directly reads
		 * past this 4-byte object and corrupts the stored server's address-
		 * family tag, which then fails dns_recv()'s RFC 5452 source-address
		 * check on every real response (silently, since a mismatch there
		 * just discards the packet). Tag it as IPv4 explicitly instead. */
		ip_addr_copy_from_ip4(dnsserver_tagged, dnsserver);
		dns_setserver(0, &dnsserver_tagged);
	}
...
/*
 * lwip<->USB-Ethernet bridge (see the file-header comment for why this
 * provides the ported driver's required network bridge).
 ...
 * CONFIG_ETHERNET==1) wires netif->linkoutput to low_level_output_mii(),
 * which calls rltk_mii_send() below with a scatter-gather list built from
 * the outgoing pbuf chain; ethernetif_mii_recv(), called from
 * usbh_comp_ecm_rxdata() below, allocates the inbound pbuf chain and calls
 * rltk_mii_recv() to fill it from usbh_comp_eth_rx_buf before handing it to
 * netif->input(). Both driver callbacks are a fixed-name contract of that
 * lwip port file - component/ethernet_mii/ethernet_usb.c implements the
 * same two functions for the legacy driver's usbh_cdc_ecm_senddata()/
 * RX_BUFFER staging (its build is excluded for usb_stack=new, see
 * application.cmake, so there is no symbol clash).
 */
#define ETHERNET_IDX (NET_IF_NUM - 1)

static u8 usbh_comp_eth_tx_buf[1536] __attribute__((aligned(CACHE_LINE_SIZE)));
static u8 *usbh_comp_eth_rx_buf;

s8 rltk_mii_send(struct eth_drv_sg *sg_list, int sg_len, int total_len)
{
	struct eth_drv_sg *last_sg;
	u8 *pdata = usbh_comp_eth_tx_buf;
	int ret;

	if ((total_len < 0) || ((u32)total_len > sizeof(usbh_comp_eth_tx_buf))) {
		return -1;
	}
	...
	ret = usbh_cdc_ecm_send_data(usbh_comp_eth_tx_buf, (u32)total_len, 1);
...
/**
  ...
  * @param  buf: RX buffer - a complete Ethernet frame (unlike the legacy
  *              driver, the ported cdc_ecm driver delivers whole frames per
  *              callback, so no RX_BUFFER chunk reassembly is needed here).
  * @param  len: RX data length (in bytes)
  * @retval Status
  */
static int usbh_comp_ecm_rxdata(u8 *buf, u32 len)
{
	usbh_comp_eth_rx_buf = buf;
...
static int usbh_comp_do_init(void)
{
	...
	for (;;) {
		...
		*/
		diag_ret = usbh_comp_dongle_diag_cmd();
		if (diag_ret == 0) {
		...
		if ((diag_ret == DONGLE_DIAG_RET_REBOOT_PENDING) &&
			(reboot_retry < CONFIG_USBH_COMP_REBOOT_PENDING_MAX_RETRY)) {
			/* Some modules only apply a change (e.g. AT+CGDCONT) after a fresh
			 * USB enumeration - tear the stack down and retry the whole
			 * bring-up from scratch instead of giving up outright. Done here
			 * (the caller of the AT state machine), not inside it, so we are
			 * not tearing down the USB stack from within its own call chain. */
			reboot_retry++;
			RTK_LOGS(TAG, RTK_LOG_WARN, "Dongle requested reconnect (%d/%d), re-init USB\n",
					 reboot_retry, CONFIG_USBH_COMP_REBOOT_PENDING_MAX_RETRY);
			usbh_dongle_ctx.in_detach = 1;
			usbh_cdc_acm_deinit();
			usbh_cdc_ecm_deinit();
		...
		if (USB_VID == vid) {  /* rtk */
			if (1 == link_is_up && (ethernet_unplug < ETH_STATUS_INIT)) {  /* unlink -> link */
				...
				if (DHCP_ADDRESS_ASSIGNED == dhcp_status) {
					netifapi_netif_set_default(&xnetif[ETHERNET_IDX]);  /* Set default gw to ether netif */
...
/*
 * The source shell-command registration mechanism has no equivalent in this
 * project, which uses
 * log_service_add_table()/log_item_t instead, see project's src/main.c) -
 * dropped rather than guessed at, along with the two commands and the GPIO
 * hot-plug trigger function that only they called. usbh_comp_gpio_init/
 * _power_init/_trigger (CONFIG_USBH_COMP_GPIO_POWER_CTRL, default off) are
 * still there if you want to wire up a replacement trigger.
 */
...
void example_usbh_composite_cdc_acm_ecm_new(void)
{
	int ret;
#if CONFIG_USBH_COMP_HOT_PLUG_TEST
...
#endif
	extern void atcmd_usb_4G_init(void);
	atcmd_usb_4G_init();
	RTK_LOGS(TAG, RTK_LOG_INFO, "USBH Composite ECM demo start\n");

	/* No separate Ethernet initialization call is needed:
	 * CONFIG_ETHERNET=1 (set for this example, see
	 * usbh_composite_cdc_acm_ecm_new.cmake) makes LwIP_Init() - already run
	 * once during this project's normal WiFi/lwip startup - register
	 * xnetif[ETHERNET_IDX] via ethernetif_mii_init() on its own. */
...
/* Forward an arbitrary AT command to the 4G dongle, e.g.
 * UECM=AT+QIDNSGIP="pool.ntp.org" - for testing whether the dongle's own
 * modem-side resolver can resolve a hostname the host's lwip DNS client
 * can't (see SETUP_NOTES.md's DNS-blocked-on-this-APN finding). */
void AUECM(void *arg)
{
	u8 *cmd = (u8 *)arg;
	u8 try_cnt = 10;

	/* Forward verbatim - do NOT run this through parse_param(), it
	 * tokenizes on ',' which most AT commands (e.g. AT+QIDNSGIP=1,"host")
	 * need to keep intact. */
	if (cmd == NULL || usbh_comp_strlen(cmd) == 0) {
		RTK_LOGS(TAG, RTK_LOG_INFO, "Usage: UECM=<AT command>\n");
		return;
	}
	...
	if (USBH_CDC_ACM_QUECTEL_DONGLE_VID == usbh_dongle_ctx.vid ||
		...
		RTK_LOGS(TAG, RTK_LOG_INFO, "No supported dongle attached\n");
	}
	RTK_LOGS(TAG, RTK_LOG_INFO, "At cmd(%s) finish!\n", cmd);
}

/* Resolve a hostname via lwip's own gethostbyname() (the same DNS path
 * example_kvs_webrtc_v2_mmf/webrtc_v2_app_src/network_transport/tcp_sockets_wrapper
 * uses via getaddrinfo()) - to test host-side DNS over the dongle's netif
 * directly, independent of any module-side (AT+QIDNSGIP) resolution.
 * e.g. UDNS=pool.ntp.org ; with no argument, defaults to pool.ntp.org. */
void AUDNS(void *arg)
{
	u8 *hostname = (u8 *)arg;
	struct hostent *server_host;
	...
	if (hostname == NULL || usbh_comp_strlen(hostname) == 0) {
		hostname = (u8 *)"pool.ntp.org";
	}

	RTK_LOGS(TAG, RTK_LOG_INFO, "Resolving %s ...\n", hostname);
	server_host = gethostbyname((char *)hostname);
	if (server_host != NULL) {
		memcpy(&addr, server_host->h_addr, 4);
		RTK_LOGS(TAG, RTK_LOG_INFO, "Resolved %s -> %s\n", hostname, inet_ntoa(addr));
	} else {
		RTK_LOGS(TAG, RTK_LOG_ERROR, "Failed to resolve %s (h_errno=%d)\n", hostname, h_errno);
	}
}

log_item_t usb_4G_items[] = {
	{"UECM", AUECM,},
	{"UDNS", AUDNS,},
};

void atcmd_usb_4G_init(void)
{
	log_service_add_table(usb_4G_items, sizeof(usb_4G_items) / sizeof(usb_4G_items[0]));
}
///////////////////
