...
#include "example_kvs_webrtc_v2_mmf.h"
...
static mm_context_t *kvs_webrtc_v2_ctx      = NULL;
...
static mm_miso_t *miso_kvs_webrtc           = NULL;
#if MEDIA_PORT_ENABLE_AUDIO_RECV
...
#define wifi_wait_time 500 // wait 5 seconds for fast connect
...
static SemaphoreHandle_t kvsWebrtcv2_stop_sem;    // wait for webrtc termination

void example_kvs_webrtc_v2_mmf_thread( void *param )
{
    atcmd_kvsWebrtc_init();
	kvsWebrtcv2_stop_sem = xSemaphoreCreateBinary();
    if (!voe_boot_fsc_status()) {
		wifi_common_init();
	}
	sntp_init();

    /* ---- Open KVS WebRTC v2 module ---- */
    kvs_webrtc_v2_ctx = mm_module_open(&kvs_webrtc_v2_module);
    if( kvs_webrtc_v2_ctx ){
        pWebrtcMmContext = kvs_webrtc_v2_ctx;
        mm_module_ctrl(kvs_webrtc_v2_ctx, MM_CMD_SET_QUEUE_LEN, 6);
        mm_module_ctrl(kvs_webrtc_v2_ctx, MM_CMD_INIT_QUEUE_ITEMS, MMQI_FLAG_STATIC );
        mm_module_ctrl(kvs_webrtc_v2_ctx, CMD_KVS_WEBRTC_SET_APPLY, 0 );
    } else{
        printf("KVS open fail");
        goto example_kvs_webrtc_v2_cleanup;
    }
    ...
    voe_heap_size = video_voe_presetting(1, V1_WIDTH, V1_HEIGHT, V1_BPS, 0,
                    ...
                    0, 0, 0 );
    ( void ) voe_heap_size;
    printf("voe heap size = %d", voe_heap_size);
    ...
    miso_kvs_webrtc  = miso_create();
    if(miso_kvs_webrtc) {
#if defined( configENABLE_TRUSTZONE ) && ( configENABLE_TRUSTZONE == 1 )
        miso_ctrl(miso_kvs_webrtc, MMIC_CMD_SET_SECURE_CONTEXT, 1, 0);
#endif
        miso_ctrl(miso_kvs_webrtc, MMIC_CMD_ADD_INPUT0, (uint32_t)video_v1_ctx, 0);
#if ( AUDIO_G711_MULAW || AUDIO_G711_ALAW )
        miso_ctrl(miso_kvs_webrtc, MMIC_CMD_ADD_INPUT1, (uint32_t)g711e_ctx, 0);
#elif AUDIO_OPUS
        miso_ctrl(miso_kvs_webrtc, MMIC_CMD_ADD_INPUT1, (uint32_t)opusc_ctx, 0);
#endif
        miso_ctrl(miso_kvs_webrtc, MMIC_CMD_ADD_OUTPUT, (uint32_t)kvs_webrtc_v2_ctx, 0);
        miso_start(miso_kvs_webrtc);
    } else {
        printf("miso_kvs_webrtc open fail");
        goto example_kvs_webrtc_v2_cleanup;
    }
    ...
    if(siso_webrtc_a2) {
        siso_ctrl(siso_webrtc_a2, MMIC_CMD_ADD_INPUT, (uint32_t)kvs_webrtc_v2_ctx, 0);
#if ( AUDIO_G711_MULAW || AUDIO_G711_ALAW )
        ...
        printf("pSisoWebrtcA2 open fail");
        goto example_kvs_webrtc_v2_cleanup;
    }

    /* Linker: decoder -> audio output */
    ...
    xSemaphoreTake(kvsWebrtcv2_stop_sem, portMAX_DELAY);

example_kvs_webrtc_v2_cleanup:

    siso_pause(siso_audio_a1);
    miso_pause(miso_kvs_webrtc, MM_OUTPUT);
#if MEDIA_PORT_ENABLE_AUDIO_RECV
    ...
    // Stop modules
    mm_module_ctrl(kvs_webrtc_v2_ctx, CMD_KVS_WEBRTC_STOP, 0);
	...
	miso_kvs_webrtc = miso_delete(miso_kvs_webrtc);
#if MEDIA_PORT_ENABLE_AUDIO_RECV
    ...
    // Close modules
    kvs_webrtc_v2_ctx = mm_module_close(kvs_webrtc_v2_ctx);
	pWebrtcMmContext = NULL;
    ...
    vSemaphoreDelete(kvsWebrtcv2_stop_sem);
	kvsWebrtcv2_stop_sem = NULL;
...
{
	if (kvsWebrtcv2_stop_sem != NULL) {
		xSemaphoreGive(kvsWebrtcv2_stop_sem);
	}
}

static log_item_t kvsWebrtcv2_items[] = {
	{"KVST", fKVST,},
};

static void atcmd_kvsWebrtc_init(void)
{
	log_service_add_table(kvsWebrtcv2_items, sizeof(kvsWebrtcv2_items) / sizeof(kvsWebrtcv2_items[0]));
}

void example_kvs_webrtc_v2_mmf(void)
{
	/*user can start their own task here*/
	if (xTaskCreate(example_kvs_webrtc_v2_mmf_thread, ((const char *)"example_kvs_webrtc_v2_mmf_thread"), 4096, NULL, tskIDLE_PRIORITY + 1, NULL) != pdPASS) {
		printf("\r\n example_kvs_webrtc_v2_mmf_thread: Create Task Error\n");
	}
}