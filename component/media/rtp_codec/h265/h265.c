...
int h265_is_i_frame(u8 *frame_buf)
{
	if ((frame_buf[4]) == H265_IFRAME) {
		return 1;
	} else {
		return 0;
	}
...
static void parse_h265_header(uint8_t *h265_data, int len, struct rtp_h265_obj *h265_obj)
{
	uint8_t *ptr = h265_data;
	int start_code_len = 0;
	int count = 0;
	while (ptr < h265_data + len - 3) {
		if (ptr[0] == 0 && ptr[1] == 0) {
			if (ptr[2] == 0 || ptr[2] == 1) {
				//NAL FOUND
				h265_obj->num_nal++;
				if (h265_obj->num_nal > MAX_NUM_NAL_PER_FRM) {
					H265_ERROR("\n\rtoo many NALs!");
					return;
				}
				start_code_len = nal_start_code_len(ptr);
				h265_obj->nal_obj[h265_obj->num_nal - 1].start_code_len = start_code_len;
				h265_obj->nal_obj[h265_obj->num_nal - 1].offset = ptr - h265_data;
				h265_obj->nal_obj[h265_obj->num_nal - 1].nal_header = ptr[start_code_len];
				h265_obj->nal_obj[h265_obj->num_nal - 1].nal_header_ext = ptr[start_code_len + 1];
				//printf("Nal type %x\r\n",h265_obj->nal_obj[h265_obj->num_nal-1].nal_header);

				//if(h265_obj->num_nal == 0x02)
				//goto EXIT;

				//h265_obj->nal_obj[h265_obj->num_nal-1].must_not_drop = 1;
				switch (h265_obj->nal_obj[h265_obj->num_nal - 1].nal_header) {
				case (H265_VPS):
				case (H265_SPS):
				case (H265_PPS):
				case (H265_IFRAME):
				...
				case (H265_SEI):
					h265_obj->nal_obj[h265_obj->num_nal - 1].must_not_drop = 1;
					h265_obj->nal_obj[h265_obj->num_nal - 1].do_not_send = 0;
					return;
					//goto EXIT;
					break;
				default:
					h265_obj->nal_obj[h265_obj->num_nal - 1].do_not_send = 1;
					h265_obj->nal_obj[h265_obj->num_nal - 1].must_not_drop = 1;
					break;
				}
...
//EXIT:
	//printf("count = %d\n",count);
	//printf("start_code_len = %d offset = %d header = %x num = %d\n",h265_obj->nal_obj[h265_obj->num_nal-1].start_code_len,h265_obj->nal_obj[h265_obj->num_nal-1].offset,h265_obj->nal_obj[h265_obj->num_nal-1].nal_header,h265_obj->num_nal);
	return;
}
...
int rtp_h265_send_unicast(struct stream_context *stream_ctx, struct rtp_object *pObject)
{
	...
	struct rtp_h265_obj *h265_obj = (struct rtp_h265_obj *)pObject->extra;
	int nal_bytes_left;
	int use_fu = FALSE;
	...
	if (pObject->rtphdr->cc > 0) { //has CSRC
		...
		if (safe_memcpy(ptr, pObject->rtphdr, RTP_HDR_SZ, ptr_end) < 0) { //prepare header
			RTP_DBG_ERROR("[H265] OVERRUN2\n\r");
			return -EAGAIN;
		}
	...
	//printf("h265_obj->num_nal = %d\r\n",h265_obj->num_nal);

	for (i = 0; i < h265_obj->num_nal; i++) {
		if (h265_obj->nal_obj[i].do_not_send) {
			continue;
		}
		rtphdr->m = 0;
		offset = 0;
		use_fu = FALSE;
		if ((i + 1) >= h265_obj->num_nal) {
			nal_bytes_left = bytes_left - h265_obj->nal_obj[i].offset;
		} else {
			nal_bytes_left = h265_obj->nal_obj[i + 1].offset - h265_obj->nal_obj[i].offset;
		}
		if (pObject->fs) {
			if (h265_obj->nal_obj[i].start_code_len > 0 && h265_obj->nal_obj[i].nal_header) {
				offset = h265_obj->nal_obj[i].start_code_len + 2;    //Devin // skip starting sequence and NAL header
			}
			nal_bytes_left -= offset;
			h265_obj->nal_obj[i].is_fu_start = TRUE;
			h265_obj->nal_obj[i].is_fu_end = FALSE;
		}

		// FU enable?
		if ((nal_bytes_left > (_write_size - header_len)) || (!pObject->fs)) {
			use_fu = TRUE;
			header_len += 3;	// append FU header
		} else {
			header_len += 2;	// append NAL header
		}
		...
		while (nal_bytes_left > 0) {
			...
			if (data_len >= nal_bytes_left) {
				...
				if (pObject->fe) {
					rtphdr->m = 1;
					h265_obj->nal_obj[i].is_fu_end = TRUE;
				}
			}

			// fill FU header
			if (use_fu) {
				ptr[0] = 49 << 1; // FU indicator  FU-A
				ptr[1] = 1;
				ptr[2] = (h265_obj->nal_obj[i].nal_header >> 1) &
						 0x3f; //ptr[1] = (h265_obj->nal_obj[i].nal_header); // FU header//(h265_obj->nal_obj[i].nal_header & 0x1f); // FU header

				if (h265_obj->nal_obj[i].is_fu_start) {
					ptr[2] |= 0x80; // first FU
					h265_obj->nal_obj[i].is_fu_start = FALSE;
				}
				if (h265_obj->nal_obj[i].is_fu_end) {
					ptr[2] |= 0x40; // final FU
					h265_obj->nal_obj[i].is_fu_end = FALSE;
				}
				if (safe_memcpy(ptr + 3, pObject->data + h265_obj->nal_obj[i].offset + offset, data_len, ptr_end) < 0) {
					RTP_DBG_ERROR("[H265] OVERRUN3\n\r");
					return -EAGAIN;
				}
				//memcpy(ptr + 3, pObject->data + h265_obj->nal_obj[i].offset + offset, data_len);
			} else {
				ptr[0] = h265_obj->nal_obj[i].nal_header;
				ptr[1] = h265_obj->nal_obj[i].nal_header_ext;

				if (safe_memcpy(ptr + 2, pObject->data + h265_obj->nal_obj[i].offset + offset, data_len, ptr_end) < 0) {
					RTP_DBG_ERROR("[H265] OVERRUN4\n\r");
					return -EAGAIN;
				}
				//memcpy(ptr + 2, pObject->data + h265_obj->nal_obj[i].offset + offset, data_len);
			}
...
check_skb:
				if (skbdata_used_num > (max_skb_buf_num - 3)) {
					...
					if (ret < 0) {
						do {
							...
							ret = sendto(socket, packet_buf, header_len + data_len, 0, (struct sockaddr *)&adr_cs, len_cs);
							if (!h265_obj->nal_obj[i].must_not_drop) {
								retry_cnt--;
							}
			...
			if (ret < 0) {
				RTP_DBG_ERROR("[H265] RTP send fail\n\r");
				return -EAGAIN;
			}
			offset += data_len;
...
int rtp_o_h265_handler(struct stream_context *stream_ctx, struct rtp_object *payload)
{
	struct rtsp_context *rtsp_ctx = stream_ctx->parent;
	struct rtp_h265_obj *h265_obj;
	...
	if (payload->fs) {
		...
		if (stream_ctx->statistics.do_start_check) {
			...
			payload->extra = malloc(sizeof(struct rtp_h265_obj));
			if (payload->extra == NULL) {
		...
		h265_obj = (struct rtp_h265_obj *)payload->extra;
		memset(h265_obj, 0, sizeof(struct rtp_h265_obj)); //clear h265_obj
		parse_h265_header(payload->data, payload->len, h265_obj);
	} else {
		if (stream_ctx->statistics.do_start_check) {
...
#if 0
	//change rate control
	if (stream_ctx->framecontrol.h265_change_rate_control) {
		if (current_tick - stream_ctx->framecontrol.change_rate_timer > stream_ctx->framecontrol.change_rate_frequency) {
			...
			if (stream_ctx->framecontrol.send_frame_count < stream_ctx->framecontrol.change_rate_threshold1) {
				if (stream_ctx->framecontrol.current_bps == 0
					...
					h265_change_bitrate(stream_ctx->framecontrol.h265_ctx, stream_ctx->bitrate / 2);
			...
			//else if(stream_ctx->framecontrol.drop_frame_count == 0)
				...
				if (stream_ctx->framecontrol.current_bps == stream_ctx->bitrate / 2) {
					h265_change_bitrate(stream_ctx->framecontrol.h265_ctx, stream_ctx->bitrate);
					stream_ctx->framecontrol.current_bps = stream_ctx->bitrate;
				}
			}
	...
	if (stream_ctx->framecontrol.start_drop_frame == 1) {
		if (h265_is_i_frame((u8 *) payload->data)) {
...
send_payload:
		...
		case (UNICAST_TCP_MODE):
			ret = rtp_h265_send_unicast(stream_ctx, payload);
			break;
		case (MULTICAST_MODE):
			//to add...
			//break;
